...
public class ActivityManagerCommonProxy {
    ...
    public static class StartActivity extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            // 宿主直通：Intent 显式指向的包，或容器内解析出的目标包命中直通列表时，
            // 不在容器内虚拟化，直接放行给真实系统，由系统拉起设备上真正安装的那个应用。
            // 典型场景：游戏内通过第三方客户端登录——容器里的客户端副本可能因自身
            // 多进程/插件架构而跑不起来。
            if (isHostPassThrough(intent, resolveInfo)) {
                Slog.d(TAG, "HostPassThrough: " + intent);
                // 注：这里试过把 callingPackage 换成 guest 包名，让宿主客户端按 callingPackage
                // 去查真正的分身签名，结果被系统直接拒绝：
                //   Permission Denial: package=<guest 包名> does not belong to uid=<宿主 uid>
                // 调用方身份由 uid 决定、不可伪造，所以只能以宿主包名发起。
                return method.invoke(who, args);
            }
         ...
         * 目标是否属于「宿主直通」应用。
         *
         * <p>判断两个来源：Intent 自身显式指定的包（setPackage / setComponent），
         * 以及容器 PackageManager 解析出来的目标包。任一命中即直通宿主。</p>
         */
        static boolean isHostPassThrough(Intent intent, ResolveInfo resolveInfo) {
            if (intent != null) {
                String pkg = intent.getPackage();
                if (pkg == null && intent.getComponent() != null) {
                    pkg = intent.getComponent().getPackageName();
                }
                if (AppSystemEnv.isHostPackage(pkg)) {
                    return true;
                }
            }
            return resolveInfo != null
                    && resolveInfo.activityInfo != null
                    && AppSystemEnv.isHostPackage(resolveInfo.activityInfo.packageName);
        }

        private Intent getIntent(Object[] args) {
            int index;
            if (BuildCompat.isR()) {                index = 3;
            } else {
                index = 2;
            }
