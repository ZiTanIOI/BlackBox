...
public interface XposedInterface {
    ...
    interface Invoker<T extends Invoker<T, U>, U extends Executable> {
         ...
         */
        // 上游此处为 sealed interface + record；BlackBox 编译环境是 Java 8（AGP 4.0），
        // 展开成等价的普通类，类名/构造器/访问器与 record 生成的二进制签名一致
        interface Type {
            /**
            ...
            final class Chain implements Type {
                ...
                @Override
                public boolean equals(Object obj) {
                    return obj instanceof Chain && ((Chain) obj).maxPriority == maxPriority;
                }

                @Override
                public int hashCode() {
                    return Integer.hashCode(maxPriority);
                }

                @Override
                public String toString() {
                    return "Chain[maxPriority=" + maxPriority + "]";
                }
            }
        }
